...
export default function Home() {
  ...
  // Create randomized image arrays for top and bottom rows
  ...
  // Initialize random image arrays on mount
  useEffect(() => {
    // Function to rotate array by random offset
    const rotateArray = (arr: string[]) => {
      const offset = Math.floor(Math.random() * arr.length);
      return [...arr.slice(offset), ...arr.slice(0, offset)];
    };

    setTopRowImages(rotateArray(allImages));
    setBottomRowImages(rotateArray(allImages));
